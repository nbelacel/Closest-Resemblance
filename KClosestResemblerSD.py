...
class KClosestResemblerSD:

    def __init__(self,t,weights = []):
        self.t = t
        self.prototypes = []
        self.weights = weights

    def _left_right_boundary(self, mu, alpha):
        left_boundary = mu - self.t * alpha
        right_boundary = mu + self.t * alpha
        return left_boundary, right_boundary

    # calculate mean value
    def _cal_mean(self,arr):
        return np.mean(arr)

    # calculate standard deviation
    def _cal_std(self, arr):
        return np.std(arr)
    ...
    def creating_prototypes(self, classes, d, ts, num_attris):
        ...
        for c in classes:
            ...
            for a in range(num_attris):
                ...
                mean = self._cal_mean(attr_a_of_c)
                std = self._cal_std(attr_a_of_c)

                # calculate the left and right boundary of the interval
                interval_l, interval_r = self._left_right_boundary(mean, std)

                # record the interval for the attribute a of class c
