...
def MLP(X, y, filename, K, d_test=None, ts_test=None, testfilename=None):
	...
	if d_test is None:
		...
		for train_index, test_index in kf.split(X,y):
			...
			clf = MLPClassifier(solver='adam', alpha=1e-5, learning_rate_init=0.01, max_iter=300, hidden_layer_sizes=(128,), random_state=412)
		...
		cross_validation.evaluation(np.array(eva_actual), np.array(eva_pred), np.unique(y),
									date.today().strftime("%d%m%Y") + '_' + filename + '_MLP_' + str(n_estimators))
		print("Total training time: %f ; Total predicting time: %f" % (np.sum(np.array(train_time)), np.sum(np.array(predict_time))))

		# print("\n***** Results *****")
		print("Average accuracy of using MLP on %s: %f" % (filename, np.mean(np.array(accuracies))))
		print("----------------------------------------------------")
	else:
		clf = MLPClassifier(solver='adam', alpha=1e-5, learning_rate_init=0.01, max_iter=300, hidden_layer_sizes=(128,), random_state=412)
		...
		cross_validation.evaluation(ts_test, pred_test, np.unique(y),
									date.today().strftime("%d%m%Y") + '_' + filename + '_' + testfilename + '_MLP_' + str(n_estimators))
		print("Total training time: %f ; Total predicting time: %f" % ((train_stop - train_start), (predict_stop - predict_start)))
		print("Average accuracy of using MLP on %s: %f" % (filename, metrics.accuracy_score(ts_test, pred_test)))
		print("----------------------------------------------------")
