...
def get_best_weights(X, y, num_attris, optimization, t, option):
    classes = np.unique(y)
    kcr = KClosestResemblerPercentile.KClosestResemblerPercentile(option)
    if (optimization == 0):
...
def KCR_percentile(X, y, filename, K, optimization, d_test=None, ts_test=None, testfilename=None, data_clean=1,validation =1,feature_selection = 0):
    ...
    best_weights = get_best_weights(X, y, num_attris, optimization, t=3, option=1)
    ...
    if d_test is None:
        ...
        best_eva_actual = []
        best_eva_pred = []
        kcr2 = KClosestResemblerPercentile.KClosestResemblerPercentile(1)
        ...
        print("\n\n---K Closest Resembler (25 & 75 percentile)--\n\n")

        print("Best Weights:\n" + str(best_weights) + "\n")
        cross_validation.evaluation(np.array(best_eva_actual), np.array(best_eva_pred), classes,
                            date.today().strftime("%d%m%Y") + '_' + filename + '_KCR_percentile_')
        print("Total training time: %f ; Total predicting time: %f" % (np.sum(np.array(train_time)), np.sum(np.array(predict_time))))
        # print("\n***** Results *****")
        print("Average accuracy of using KCR (25 & 75 percentile) on %s: %f" % (filename, accuracy_score(np.array(best_eva_actual), np.array(best_eva_pred))))
        print("----------------------------------------------------")
        ...
        print("\n\n---K Closest Resembler (25 & 75 percentile)--\n\n")
        print("Best Weights:\n" + str(best_weights) + "\n")
        cross_validation.evaluation(ts_test, pred_test, np.unique(y), date.today().strftime("%d%m%Y") + '_' + filename + '_' + testfilename + '_KCR_percentile_')
        print("Total training time: %f ; Total predicting time: %f" % ((train_stop - train_start), (predict_stop - predict_start)))
        print("Average accuracy of using KCR (25 & 75 percentile) on %s: %f" %(filename, accuracy_score(ts_test, pred_test)))
        print("----------------------------------------------------")
