...
def RGLVQ_Classifier(X, y, filename, K, d_test=None, ts_test=None, testfilename=None):
	# loo = LeaveOneOut()
    if (K <= 1):
        kf = LeaveOneOut()
    else:
    ...
    if d_test is None:
        ...
        for train_index, test_index in kf.split(X,y):
            ...
            clf = RGLVQ(number_of_prototypes)
        ...
        cross_validation.evaluation(np.array(eva_actual), np.array(eva_pred), np.unique(y),
									date.today().strftime("%d%m%Y") + '_' + filename + '_RGLVQ_')
        print("Total training time: %f ; Total predicting time: %f" % (
			np.sum(np.array(train_time)), np.sum(np.array(predict_time))))

		# print("\n***** Results *****")
        print("Average accuracy of using RGLVQ on %s: %f" % (filename, np.mean(np.array(accuracies))))
        print("----------------------------------------------------")
    else:
        ...
        cross_validation.evaluation(ts_test, pred_test, np.unique(y),
									date.today().strftime("%d%m%Y") + '_' + filename + '_' + testfilename + '_RGLVQ_' )
        print("Total training time: %f ; Total predicting time: %f" % ((train_stop - train_start), (predict_stop - predict_start)))
        print("Average accuracy of using RGLVQ on %s: %f" % (filename, metrics.accuracy_score(ts_test, pred_test)))
        print("----------------------------------------------------")
